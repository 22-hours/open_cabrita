...
export EXP_NAME=mc4-pt-clean_text-gemma-7b-1
export MODEL_DIR=${GCS_BUCKET:-gs://your-bucket-name}/gemma_models/${EXP_NAME}
export HUGGING_FACE_HUB_TOKEN='TOKEN HERE'
export WANDB_API_KEY='API KEY HERE'
export WANDB_ENTITY="PROJECT ENTITY HERE"
...
(python -m EasyLM.models.gemma.gemma_train \
    ...
    --eval_freq=25000 \
    --eval_batches=100 \
    --load_checkpoint="params::${HOME}/gemma7beasylm" \
    --tokenizer.vocab_file="${HOME}/gemma7bhf/tokenizer.model" \
    --optimizer.type='adamw' \
    --optimizer.accumulate_gradient_steps=64 \
    --optimizer.adamw_optimizer.weight_decay=0.1 \
    --optimizer.adamw_optimizer.lr=1e-4 \
    --optimizer.adamw_optimizer.end_lr=3e-5 \
    --optimizer.adamw_optimizer.lr_warmup_steps=1500 \ #~1% \
    --optimizer.adamw_optimizer.lr_decay_steps=150000 \
    --optimizer.adamw_optimizer.bf16_momentum=False \
    ...
    --train_dataset.huggingface_dataset_v2.batch_size=8 \
    --train_dataset.huggingface_dataset_v2.seq_length=8192 \
    ...
    --eval_dataset.huggingface_dataset_v2.batch_size=8 \
    --eval_dataset.huggingface_dataset_v2.seq_length=8192 \
    ...
    --logger.wandb_dir="$HOME/experiment_output/llama2_7b"
) 2>&1 | tee -a $LOG_FILE
